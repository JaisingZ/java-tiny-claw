...
如果 `agent.properties` 中配置了 `telegram.webhook.enabled=true`，无参数启动会进入 Telegram Webhook 常驻服务；否则只打印 harness 信息。
...
`telegram` 是显式通信服务子命令，不依赖 `telegram.webhook.enabled`，也不接受 `--debug` 等额外参数。
