...
- Subagent：`spawn_subagent` 同步拉起只读子 Agent，仅挂载 `read_file`，用于受限探索。
...
src/main/java/io/github/tinyclaw/agent
  ...
  runtime/         AgentEngine, RunLogger, RunResult, memory, compaction
  tool/            tool interface, registry, built-in tools, permissions

src/test/java/io/github/tinyclaw/agent
  app/ architecture/ communication/ context/ provider/ runtime/ tool/
...
CLI Plan Mode 状态目录为 `.tinyclaw/state/cli/default/`。
